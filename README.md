...
| **Servers & channels** | MLS group per channel (host is sole committer); create server / channel, direct invites + invite links, **server-level join password**, roles & permissions v1 (**custom-emoji role icons**), member kick + inactivity auto-kick, channel history, delete server / channel, leave channel, rename channel, always-present `#general`, public **discovery** & join, **per-server nicknames (host-set or member-requested)** |
| **In-channel** | Emoji reactions (unicode + custom), **categorised emoji picker**, edit / delete, replies, pinned messages, @mentions, message forwarding, per-conversation unread counts + mute, **custom per-server emoji / stickers / soundboards**, opt-in **GIF search** (Tenor/Giphy, off unless the operator configures a key and the user opts in) |
...
| [`crates/dante-sfu`](crates/dante-sfu/README.md) | SFU media component: DTLS-SRTP per participant, opaque RTP forwarding (tested; relay-hosted behind the `sfu` feature) |
