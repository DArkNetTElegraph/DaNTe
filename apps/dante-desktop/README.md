...
**Linux + Wayland:** two known WebKitGTK issues under Wayland, neither in
DaNTe's own code:

- If the window never opens and you see `GTK-CRITICAL:
  gtk_widget_get_scale_factor: assertion 'GTK_IS_WIDGET (widget)' failed`
  followed by `Gdk-Message: Error 71 (Protocol error) dispatching to Wayland
  display`, force GTK's X11 backend instead (via XWayland, present on
  effectively every Wayland session):

  ```sh
  GDK_BACKEND=x11 cargo tauri dev
  ```

- If the window opens but its content area stays black, with `Failed to
  create GBM buffer of size WxH: Invalid argument` in the log, that's
  WebKit's DMA-BUF renderer failing on your GPU driver/Mesa combination —
  disable it too:
...
`scripts/setup-linux.sh --desktop` sets both automatically when it detects
a Wayland session.
