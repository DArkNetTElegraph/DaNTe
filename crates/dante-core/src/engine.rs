...
impl Engine {
    ...
    pub async fn refresh_mls_key_package(&mut self) -> Result<(), CoreError> {
        ...
        while self.mls_pending.len() + fresh.len() < Self::MLS_KEYPKG_POOL {
            let (pending, kp) = mls::Member::publish_key_package(&me, self.identity.signing_key())
                .map_err(mls_err)?;
    ...
    pub async fn start_group_call(
        ...
        let mut mls_member =
            mls::Member::create(&me, self.identity.signing_key(), channel_id).map_err(mls_err)?;
        ...
        for m in &others {
            if let Ok(Some(bytes)) = sync::get_key_package(&mut self.client, m).await {
                ...
                if !self.key_package_binding_is_valid(&mls_member, &kp, m) {
                    continue;
                }
                kps.push(kp);
                invited.push(*m);
    ...
    pub async fn join_voice_channel(
        ...
        if present.is_empty() {
            ...
            let member = mls::Member::create(&me, self.identity.signing_key(), channel_id)
                .map_err(mls_err)?;
            self.group_calls
                .insert(*channel_id, GroupCall { mls: member });
            let _ = self.refresh_mls_key_package().await;
    ...
    /// Whether `kp` is safe to trust enough to add its holder as `peer_id`:
    /// its DaNTe credential binding must verify (`key_package_identity`, in
    /// `dante-mls` — self-consistency only, no ledger access there), the
    /// identity it names must equal `peer_id`, and — the check `dante-mls`
    /// can't do itself — the `idk_pub` that signed the binding must be the
    /// ledger's own current signing key for `peer_id`. Without this last
    /// step, a self-consistent binding proves nothing: anyone can mint their
    /// own real `idk` and sign a credential claiming to be someone else's
    /// identity bytes; only the ledger says which `idk` is actually
    /// authoritative for a given identity.
    fn key_package_binding_is_valid(
        &self,
        mls: &mls::Member,
        kp: &mls::KeyPkg,
        peer_id: &[u8; 32],
        ...
        identity == peer_id.as_slice() && self.ledger.idk_for_id(peer_id) == Some(idk_pub)
    }
    ...
    async fn mls_add_member(
        ...
        let (commit, welcome, info, since_seq, server_root, log_key) = {
            ...
            // the credential's own binding must verify, name this exact
            // peer, and be signed by the ledger's own current key for them
            // (a self-consistent binding alone just proves *some* real idk
            // vouched for it — the ledger says which idk actually is that
            // peer).
            let (credential, idk_pub) = ch.mls.key_package_identity(&kp).map_err(mls_err)?;
            let ledger_idk = self.ledger.idk_for_id(peer_id);
            if credential != peer_id.as_slice() || ledger_idk != Some(idk_pub) {
                return Err(CoreError::Channel(
                    ...
                    if issue {
                            ...
                            sync::get_key_package(&mut self.client, &requester).await
                        {
                            ...
                            let hs = self.group_calls.get_mut(&channel_id).and_then(|gc| {
                                let Ok((identity, idk_pub)) = gc.mls.key_package_identity(&kp)
                                else {
                                    return None;
                                };
                                if identity != requester.as_slice() || ledger_idk != Some(idk_pub) {
                                    return None;
                                }
                                gc.mls.add(&[kp]).ok()
                            });
