//! SSRF-guarded link-preview fetcher (feature `unfurl`).
//!
//! Shared by `dante-cli`'s own local unfurler (`dante serve`'s opt-in
//! `POST /api/embeds`) and `dante-relay`'s opt-in relay-side one — moved here
//! from `dante-cli/src/unfurl.rs` so the two don't maintain separate copies
//! of the same SSRF-critical fetch path. This file's logic (and the SSRF
//! guard specifically) is otherwise unchanged from its original location.
//!
//! Privacy note: fetching a URL's metadata reveals the fetcher's IP address
//! to the linked site — the local client's IP for `dante-cli`'s own use, or
//! the relay operator's IP when a relay does this on a client's behalf. Every
//! connection target (the initial host and every redirect hop) is checked to
//! be a public unicast address, so a hostile link cannot point the fetcher at
//! a loopback / LAN / cloud-metadata endpoint (SSRF): no cookies, no
//! JavaScript, capped body size and wall-clock time.
...
/// `dante-cli`'s `gifsearch` module's CDN check) must not have it silently
/// bypassed by a redirect the allowlisted host itself returns; `unfurl`'s own
/// arbitrary-URL use passes `None` since any public host is legitimately in
/// scope for it.
pub async fn fetch(
    start_url: &str,
    deadline: Instant,
