...
- **Optional per-server auto-kick:** admin-set inactivity window (default off); prunes the local membership list only. *(not built — needs the inactivity tracking; the removal mechanism above is the primitive it would drive.)*
