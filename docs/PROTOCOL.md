...
| 8 | `IdentityProfile` | `{ avatar_hash: opt<[u8;32]>, status: opt<string(<=256)> }` — mutable public identity state. `author` **must** be the current chain tip; `avatar_hash` is the SHA-256 of the global avatar image in the relay blob store, `status` is a free-text bio with no control characters, and absent either field means "none". Wire note: the avatar keeps its original `bool` + `[u8;32]` layout and the status is appended as a trailing string, so an avatar-only body — and every record from the first release — decodes unchanged; no trailing bytes means no status. Accepted only for a known, non-evaporated, non-revoked chain whose newest accepted `IdentityProfile` has a strictly smaller `created_ms` (so an old profile cannot be replayed over a newer one). Does **not** count as liveness activity — a PoW-free profile update cannot extend an identity's evaporation TTL. |
